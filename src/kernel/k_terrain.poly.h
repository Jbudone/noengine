#ifndef __K_TERRAIN_POLY_H__
#define __K_TERRAIN_POLY_H__
 ...
 *  		JournalEntry {
	...
	CSG
		...
		CSG will work in two modes: map editor mode, and game mode. Both will work essentially the same way, however both will work with different CSG shapes (primitives for map editing, dynamic polygons for game mode)


		PROCESS
			- Find all tri's intersected by the convex hull of CSG (check by edges of tri)
			- Find all intersection points of CSG on tri's; eg. if sphere hits tri at 2 points on a line, there may be some error introduced if we simply connect those two points by a line. Check this line against the CSG object and add more points as necessary
			- For every vertex inside of the convex hull (not the surface), push it inwards to the new expected point. Apply the CSG direction (view forward vector) to the vertex, and find the CSG surface hitpoint. (NOTE: some verts may go to the same hitpoint, eg. if doing CSG along a flat hilltop)
			- Check all affected edges from the push, how does the edge compare to the CSG? Is its midpoint far from the surface? If so then subdivide the edge and move its midpoint to the surface; repeat as necessary
			- Delaunay re-triangulation

		COLLISIONS
			- Check vert normals (NOTE: verts may have multiple normals, one for each triangle to which its associated); does vert normal point in direction of CSG? If so then count this vert as a collision vertex
			- Delay all collision detection until the end (before re-triangulation)
			- Check the moved verts (paths? tri's?) against collision tri's (planes?); if they intersect, then remove this vert. 
			- Remove all collision verts

		QUESTIONS
			- How to handle collisions? How to connect properly (can we depend on Delaunay?)
			- Noise in CSG
...
public:
	...
	TerrainPoly(GLuint gl); // TODO: how to handle shaders?
	~TerrainPoly();
	void clearTerrain();
	...
	void CSG(Vertex position, Vertex direction);
	...
	vector<iTrianglePoly> debugTriangles; // TODO: remove or implement debug mode properly
...
struct iTrianglePoly {
	iTrianglePoly(ushort p0, ushort p1, ushort p2) : p0(p0), p1(p1), p2(p2) { }
	ushort p0, p1, p2;
	// uchar metadata;
	bool operator ==(iTrianglePoly& triangle) { 
...
struct Voxel {
	...
	Chunk* chunk;
	TerrainPoly* terrain;
	float getX() { return (terrain->vertexBuffer[vertexIndex].x); }
	float getY() { return (terrain->vertexBuffer[vertexIndex].y); }
	float getZ() { return (terrain->vertexBuffer[vertexIndex].z); }
};
struct Chunk {
	...
	vector<iTrianglePoly> triangleBuffer;
	// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
struct EdgeTriTree {
	...
	static TerrainPoly* terrain;
	EdgeTriTree(TerrainPoly* terrain);
	~EdgeTriTree();
	...
	struct EdgeChunk {
		...
		static void addMissingEdgeChunks(vector<EdgeChunk*>* chunks, EdgeChunk* chunk, ushort vertexIndex, TerrainPoly* terrain ) {
			
			float x = terrain->vertexBuffer[vertexIndex].x;
			float y = terrain->vertexBuffer[vertexIndex].y;
			float z = terrain->vertexBuffer[vertexIndex].z;
...
struct TerrainSelection {
	struct SelectionClass {
		...
		Chunk* refChunk;
		int refTri_id;
		vector<iTrianglePoly> triangles;
	};
