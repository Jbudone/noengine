...
*/
struct Texture {
	Texture(const char* filename, const uchar* imageData, int width, int height, uint description = 0);
	...
	uint description;
	static const uint TEX_CHAN_ALPHA = 1 << 0;
	static Texture* loadTexture(const char* filename, uint description = 0);
};
